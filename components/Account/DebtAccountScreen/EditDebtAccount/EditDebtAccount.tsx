...
export default function EditDebtModal({
  ...
  console.log(account.associated);
  const handleEditDebt = async (values: any) => {
    ...
    if (account.associated === values.associated) {
      const associatedAccountRef = doc(
        db,
        'users',
        session.data?.user?.email as string,
        'accounts',
      ...
      await updateDoc(associatedAccountRef, { amount: newAmount });
        ...
        const newAssociatedAccountRef = doc(
            db,
            'users',
            session.data?.user?.email as string,
            'accounts',
        ...
        const associatedAccount = await getDoc(associatedAccountRef);
        const newAssociatedAccount = await getDoc(newAssociatedAccountRef);
        const associatedAccountData = associatedAccount.data();
        const newAssociatedAccountData = newAssociatedAccount.data();

        const newAmount =
            associatedAccountData?.amount - Math.abs(account.amount);
        
        const newNewAmount =
            newAssociatedAccountData?.amount + Math.abs(values.amount);
        
        await updateDoc(associatedAccountRef, { amount: newAmount });
        await updateDoc(newAssociatedAccountRef, { amount: newNewAmount });
